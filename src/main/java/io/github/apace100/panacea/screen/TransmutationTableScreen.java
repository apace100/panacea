...
public class TransmutationTableScreen extends HandledScreen<TransmutationTableScreenHandler> {
   ...
   protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
      ...
      if (this.canCraft) {
         ...
         List<Item> list = ((TransmutationTableScreenHandler)this.handler).getAvailableRecipes();

         for(int l = this.scrollOffset; l < k && l < ((TransmutationTableScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
   ...
   private void renderRecipeBackground(MatrixStack matrixStack, int i, int j, int k, int l, int m) {
      for(int n = this.scrollOffset; n < m && n < ((TransmutationTableScreenHandler)this.handler).getAvailableRecipeCount(); ++n) {
         int o = n - this.scrollOffset;
         int p = k + o % 4 * 16;
         int q = o / 4;
         int r = l + q * 18 + 2;
         int s = this.backgroundHeight;
         if (n == ((TransmutationTableScreenHandler)this.handler).getSelectedRecipe()) {
            s += 18;
   ...
   private void renderRecipeIcons(int x, int y, int scrollOffset) {
      ...
      for(int i = this.scrollOffset; i < scrollOffset && i < ((TransmutationTableScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      if (this.canCraft) {
         ...
         for(int l = this.scrollOffset; l < k; ++l) {
            ...
            if (d >= 0.0D && e >= 0.0D && d < 16.0D && e < 18.0D && ((TransmutationTableScreenHandler)this.handler).onButtonClick(this.client.player, l)) {
               MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_STONECUTTER_SELECT_RECIPE, 1.0F));
               this.client.interactionManager.clickButton(((TransmutationTableScreenHandler)this.handler).syncId, l);
               return true;
            }
         }
   ...
   private boolean shouldScroll() {
      return this.canCraft && ((TransmutationTableScreenHandler)this.handler).getAvailableRecipeCount() > 12;
   }

   protected int getMaxScroll() {
      return (((TransmutationTableScreenHandler)this.handler).getAvailableRecipeCount() + 4 - 1) / 4 - 3;
   }

   private void onInventoryChange() {
      this.canCraft = ((TransmutationTableScreenHandler)this.handler).canCraft();
      if (!this.canCraft) {
