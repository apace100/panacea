...
public class TransmutationTableScreenHandler extends ScreenHandler {
   ...
   public TransmutationTableScreenHandler(int syncId, PlayerInventory playerInventory, final ScreenHandlerContext context) {
      super(ModScreenHandlers.TRANSMUTATION_TABLE, syncId);
      TAGS = new LinkedList<>();
      for(int i = 0; i < TransmutationTableBlock.TRANSMUTABLE.length; i++) {
         Tag<Item> tag = TagRegistry.item(TransmutationTableBlock.TRANSMUTABLE[i]);
         TAGS.add(tag);
      }
      ...
      this.outputSlot = this.addSlot(new Slot(this.output, 1, 143, 33) {
         ...
         public ItemStack onTakeItem(PlayerEntity player, ItemStack stack) {
            ...
            context.run((world, blockPos) -> {
               ...
               if (TransmutationTableScreenHandler.this.lastTakeTime != l) {
                  world.playSound((PlayerEntity)null, blockPos, SoundEvents.BLOCK_BEACON_POWER_SELECT, SoundCategory.BLOCKS, 1.0F, 1.0F);
                  TransmutationTableScreenHandler.this.lastTakeTime = l;
               }
   ...
   public boolean canUse(PlayerEntity player) {
      return canUse(this.context, player, ModBlocks.TRANSMUTATION_TABLE);
   }
   ...
   private void updateInput(Inventory input, ItemStack stack) {
      ...
      if (!stack.isEmpty()) {
         ...
         TAGS.stream().filter(t -> stack.getItem().isIn(t)).map(Tag::values).forEach(availableRecipes::addAll);
      }
   }
   ...
   public ScreenHandlerType<?> getType() {
      return ModScreenHandlers.TRANSMUTATION_TABLE;
   }
   ...
   public ItemStack transferSlot(PlayerEntity player, int index) {
      ...
      if (slot != null && slot.hasStack()) {
         ...
         } else if (this.isTransmutable(itemStack2)) {
   ...
   private boolean isTransmutable(ItemStack stack) {
      if(TAGS == null) {
         TAGS = new LinkedList<Tag<Item>>();
         boolean found = false;
         for(int i = 0; i < TransmutationTableBlock.TRANSMUTABLE.length; i++) {
            Tag<Item> tag = TagRegistry.item(TransmutationTableBlock.TRANSMUTABLE[i]);
            TAGS.add(tag);
            if(!found && tag.contains(stack.getItem())) {
               found = true;
            }
         }
         return found;
